...
void prepare_inputs(
	...
	std::shared_ptr<kc::InferResult> result_placeholder = nullptr;
	FAIL_IF_ERR(
		client->StartStream(
	...
	for (size_t i = 0; i < repeat_cnt; i++) {
		...
		FAIL_IF_ERR(client->AsyncStreamInfer(options, inputs), "unable to run tokenize model");


		bool received_final = false;
		while (!received_final) {
			std::unique_lock<std::mutex> lk(mutex_);
			cv_.wait(lk, [&] { return result_placeholder != nullptr; });

			if (result_placeholder->RequestStatus().IsOk()) {
				result_placeholder->IsFinalResponse(&received_final);
		...
		std::cout << "\nTotal inference time: "
				<< total_duration.count() / 1000.0f << " ms"
				<< std::endl;
	}
	client->StopStream();
...
void test_stream_infer(
    const std::unique_ptr<kc::KyTensorClient>& client,
    const std::string& model_name,
    const std::string& prompt) {

	prepare_inputs(client, model_name, RequestType::COMPLETIONS, "chat-123", prompt,
		0.7f, 40, 0.95f, 0, 50, true, "<|im_end|>", true, std::vector<float>(), 1.1f);
...
int main(int argc, char** argv) {
  ...
  int opt;
	...
	test_stream_infer(client, model_name, "写一篇100字的关于人工智能的文章");
